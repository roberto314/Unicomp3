...
def upload_image(ser, fpath, cf, name):
	try:
		...
		try:
			...
			imgstart = None
			print(f'start not given. Calculating size')
		try:
			imgend  = int(cf[name]['end'] , 0)
		except:
			imgend = None
			print(f'end not given. Calculating size')
		if imgstart == None and imgend == None:
			print(f'{bcolors.FAIL}Need at least start or end!{bcolors.ENDC}')
			exit()
		if imgstart == None or imgend == None: # one is there
			if imgstart != None:
				imgend = imgstart + fsize - 1
			else:
				imgstart = imgend - fsize + 1
		...
		print(f'{bcolors.FAIL}Error in upload_image: {e}!{bcolors.ENDC}')
		return 1
#------------------------------------------
...
if __name__ == '__main__':
	...
	ser = None
	try:
		ser = Serial(port, 921600, timeout = 12, writeTimeout = 1)
	except IOError:
		print(f'{bcolors.FAIL}############################################{bcolors.ENDC}')
